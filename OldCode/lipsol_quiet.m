function [xsol,fval,lambda,exitflag,output] = lipsol_quiet(f,Aineq,bineq,Aeq,beq,lb,ub,options,defaultopt,computeLambda)
...
%   Copyright 1990-2014 The University of Maryland Baltimore County and The MathWorks, Inc.
...
% cholinf(A) only accepts sparse matrices.
if (~issparse(A))
   A = sparse(A);
end
f = sparse(f);
...
if ~converged && iter == maxiter+1
   theMessage = sprintf('Maximum number of iterations exceeded; increase options.MaxIterations.');
   exitflag = 0;
   iter = iter - 1; 
end   
...
if minrp < tol
   ...
   msg = sprintf(['         the dual appears to be infeasible (and the primal unbounded).' ,...
              '      \n         (The primal residual < OptimalityTolerance=%3.2e.)'],tol);
   return
end
if minrd < tol
   exitflag = -2;
   msg = sprintf(['         the primal appears to be infeasible (and the dual unbounded).', ...
              '\n         (The dual residual < OptimalityTolerance=%3.2e.)'],tol);
   return
end
...
if ((minrp < tol10) && (minrd > sqrttol))
   ...
   msg = sprintf(['         the dual appears to be infeasible (and the primal unbounded) since', ...
         '\n         the dual residual > sqrt(OptimalityTolerance)=%3.2e.' ,...
         '\n         (The primal residual < 10*OptimalityTolerance=%3.2e.)'], ...
      sqrttol,tol10);
   return
end
if ((minrd < tol10) && (minrp > sqrttol))
   ...
   msg = sprintf(['         the primal appears to be infeasible (and the dual unbounded) since', ...
             '\n         the primal residual > sqrt(OptimalityTolerance)=%3.2e.', ...
             '\n         (The dual residual < 10*OptimalityTolerance=%3.2e.)'], ...
          sqrttol,tol10);
   return
end
