...
% % those:
% Anan = isnan(A1D); %find points that are not allowed on 2D grid
% Anew = Anan<1;
% gtest = g1D.xs{1}.*Anew; %removed not-allowed points from grid
% gtest_rem = g1D.xs{1}.*Anan; %removed not-allowed points from grid
% 
% 
